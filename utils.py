...
from pdkcore import Card, Player
...
def paodekuai_reorganize(trajectories):
    ''' Reorganize the trajectory to make it RL friendly

    Args:
        trajectories (list): A list of trajectories
        reward_seq (list): A list of payoffs for the players. Each entry corresponds to one player
    ...
    for player in range(player_num):
        for i in range(0, len(trajectories[player])-1, 3):
            # i is the index of transition
            if i == len(trajectories[player])-4:
              #  reward = reward_seq[player]
                done = True
            else:
                done = False
           # print(i)
